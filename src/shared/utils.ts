...
import { ISearchParamBase } from '@core/models/pagination';
...
export function controllerPaginationHelper (query: qs.ParsedQs): ISearchParamBase {
  return {
    offset: query.offset
...
export function addLoggers (
    ...
    console.log('⚠️ Operation logger: ', JSON.stringify({
      ...
      request: {
        ...
        body: !overshadow
          ...
          : requestData && encodeBase64(JSON.stringify(requestData)),
  ...
  instance.interceptors.response.use(
      ...
      console.log('⚠️ Operation logger: ', JSON.stringify({
        ...
        response: {
          ...
          data: !overshadow
            ...
            : responseData && encodeBase64(JSON.stringify(responseData)),
        },
      }));
    ...
    (err) => {
      ...
      console.log('⚠️ Operation error logger: ', JSON.stringify({
        origin,
        type: 'error',
        req: {
          id: requestIdentifier,
        },
        err: handleError(errorData, true),
      }));
...
export function readTemplate (path: string, encoding: string): Promise<unknown> {
  const promise = new Promise((resolve, reject) => {
    readFile(path, encoding as unknown, (err, template) => {
      if (err) {
        reject(new BusinessError(BusinessErrorCodes.TEMPLATE_NOT_FOUND));
      }
